...
class ColourDetectorNode(Node):
    ...
    def _detect(self, bgr: np.ndarray) -> None:
        ...
        for colour, ranges in _COLOUR_RANGES.items():
            ...
            for contour in contours:
                ...
                cx_px      = x + w // 2   # bbox centre — used by perception_adapter for depth
                cy_px      = y + h // 2

                confidence = min(1.0, area / 5000.0)
                img_path   = self._maybe_save_photo(bgr, colour, x, y, w, h)

                self._publish(colour, confidence, img_path, cx_px, cy_px)
    ...
    def _maybe_save_photo(
        ...
        return self._save_photo(bgr, colour, x, y, w, h)
