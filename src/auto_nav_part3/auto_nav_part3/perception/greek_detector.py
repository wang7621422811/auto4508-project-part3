...
class GreekDetectorNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # label -> [(robot_x, robot_y), ...] 已存图的机器人位置列表
    ...
    def _detect(self, bgr: np.ndarray) -> None:
        ...
        # Step 1 — find white paper on bucket
        # 没有检测到白色纸板 → 整个场景不含希腊字母标记，直接跳过
        # 原逻辑 fallback 到整帧图像会对 Gazebo 彩色方块误识别为字母
        result = self._detect_paper(bgr)
        if result is None:
            return
        paper, cx_px, cy_px = result
        src = paper
        ...
        # Step 4 — 新空间位置才存图：防止同一物体反复写入，又能检测不同位置的相同字母
        img_path = ""
        if self._is_new_location(label):
            ...
            img_path = self._save_photo(bgr, paper, label)
            positions = self._saved_positions.setdefault(label, [])
            positions.append((self._robot_x, self._robot_y))
        self._publish(label, confidence, img_path, cx_px, cy_px)
    ...
    @staticmethod
    def _detect_paper(bgr: np.ndarray) -> Optional[np.ndarray]:
        """
        Find the white paper rectangle on the bucket and return a
        ...
        for cnt in contours:
            ...
            if area < 0.03 * h * w:
                continue
            if area > 0.40 * h * w:  # reject background (too large to be paper)
                continue
            _bx, _by, _bw, _bh = cv2.boundingRect(cnt)
            if _by <= 3:  # top edge touching image top = sky/background
                continue
    ...
    def _save_photo(
        self,
        bgr: np.ndarray,
        paper: Optional[np.ndarray],
        label: str,
    ) -> str:
        annotated = bgr.copy()
        # Draw paper outline if detected
