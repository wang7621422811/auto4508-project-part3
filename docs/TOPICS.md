...
| `/odom` | `nav_msgs/msg/Odometry` | subscribe | Mapping / Planning / Logging / Perception | Robot pose estimate. Used by perception nodes to tag detection location. |
...
| `/oak/rgb/image_raw` | `sensor_msgs/msg/Image` | subscribe | Perception | OAK-D colour camera feed 640x480 BGR at 15fps. Used by colour_detector and greek_detector. |
...
| `/part3/perception/logger_status` | `std_msgs/msg/String` | Perception member | UI / logger | Running count of detections and manifest path. | `detections=5 greek=3 colour=2 manifest=artifacts/markers/manifest.csv` |
...
## Perception node interfaces

### Event format
All marker events on `/part3/perception/marker_event` follow this exact format:

### Perception nodes summary
| Node | Subscribes | Publishes | Purpose |
|---|---|---|---|
| `colour_detector` | `/oak/rgb/image_raw`, `/odom` | `/part3/perception/marker_event` | Detects red and yellow obstacles using HSV masking |
| `greek_detector` | `/oak/rgb/image_raw`, `/odom` | `/part3/perception/marker_event` | Detects hand-drawn Greek letters using ONNX classifier |
| `photo_logger` | `/part3/perception/marker_event` | `/part3/perception/logger_status` | Logs all detections to CSV manifest and copies photos |

### Artifact outputs
| Path | Owner | Purpose |
|---|---|---|
| `artifacts/perception_photos/` | `colour_detector`, `greek_detector` | Annotated photos saved at moment of detection |
| `artifacts/markers/manifest.csv` | `photo_logger` | One row per detection: timestamp, type, label, x, y, confidence, range_m, image paths |
| `artifacts/markers/photos/` | `photo_logger` | Copies of all detection photos for report evidence |

### Manifest CSV columns
`timestamp, marker_type, label, x, y, confidence, range_m, image_path, photo_copy_path`
